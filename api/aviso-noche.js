import { db, transporter, GMAIL, fechaLabel, esTurnoNoche, getCorreosHD } from './_helpers.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pendientes = registrosNoche.filter(p => !p.salida || p.estado !== 'salida')

    if (!pendientes.length) {
      return res.status(200).json({ ok: true, enviado: false, message: 'Sin pendientes' })
    }
      ...
      <div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto">
        ...
        <div style="background:#fff8f0;padding:20px 24px;border-radius:0 0 8px 8px;border:1px solid #fde68a;border-top:none">
          <p style="color:#1e2433;font-size:14px;margin:0 0 16px">
            Hay <strong>${pendientes.length} persona${pendientes.length>1?'s':''} sin salida registrada</strong> del ${turnoLabel}.
            Tienes <strong>10 minutos</strong> para regularizar antes del reporte final.
          </p>
          ${secciones}
          <p style="color:#92400e;font-size:12px;margin:16px 0 0">Entra al panel y registra las salidas pendientes antes de las 10:00am.</p>
        </div>
      </div>`
